...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            acl.values,  # Fixture needs acl
            storage_class.values[0],  # Fixture needs storage class
            cmd.values[0]["command"],
            storage_class.values[0]['expected'],
            marks=cmd.marks,
            id=f"{storage_class.id}-{cmd.id}-{acl.id}"
        ) for acl, storage_class, cmd in product(acl_list, storage_classes_list, commands)
    ],
    indirect=["fixture_bucket_with_name"]
)
def test_upload_cold_storage_cli(s3_client, active_mgc_workspace, profile_name, fixture_bucket_with_name, fixture_create_small_file, storage_class, cmd_template, expected):
    """
    ...
    try:
        ...
        # Validate the uploaded acl_listobject
