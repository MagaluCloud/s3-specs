...
test_cases_policy_eval = [
    # Cenário 1: [Allow1, Allow2] - Ambos dao match → ALLOW
    (
        {"number_clients": 1},
        {
            "Version": "2012-10-17",
        ...
        "get_object",
        "allow",  # Resultado esperado
    ),
    # Cenário 2: [Deny1, Allow1] - Ambos dao match → DENY (Deny aparece primeiro)
    (
        {"number_clients": 1},
        {
            "Version": "2012-10-17",
        ...
        "get_object",
        "deny",  # Resultado esperado
    ),
    # Cenário 3: [Allow1, Deny1] - Ambos dao match → DENY (Deny tem precedência)
    (
        {"number_clients": 1},
        {
            "Version": "2012-10-17",
    ...
    # Cenário 4: [Deny1, Deny2] - Ambos dao match → DENY (Primeiro Deny para a avaliação)
    (
        {"number_clients": 1},
        {
            "Version": "2012-10-17",
    ...
    # Cenário 6: [Allow1, Deny1] - Apenas Allow da match → ALLOW (Deny não da match por condição IP)
    (
        {"number_clients": 1},
        {
            "Version": "2012-10-17",
        ...
        "get_object",
        "allow",  # Resultado esperado - Deny não da match pois IP não é 203.0.113.42
    ),
    # Cenário 7: [Allow1, Deny1] - Apenas Deny da match → DENY (Allow não bate por condição IP)
    (
        {"number_clients": 1},
        {
            "Version": "2012-10-17",
        ...
        "get_object",
        "deny",  # Resultado esperado - Allow não da match por IP, Deny da match
    ),
]
...
@pytest.mark.parametrize(
    ...
    ids=[
        "scenario_1_allow_allow_both_match",
        "scenario_2_deny_allow_both_match",
        "scenario_3_allow_deny_both_match",
        "scenario_4_deny_deny_both_match",
        "scenario_6_allow_deny_only_allow_matches",
        "scenario_7_allow_deny_only_deny_matches",
    ],
)
def test_policy_evaluation_conditions(
    multiple_s3_clients,
    ...
    else:  # expected_result == "deny"
        # Esperamos que a operação seja negada
        try:
            response = method(**kwargs)
