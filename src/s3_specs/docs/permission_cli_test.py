...
def test_list_objects(active_mgc_workspace_second_env, fixture_public_bucket, cmd_template, profile_name_second):
...
cli_commands = [
    pytest.param({
        "cli": "mgc",
        "canned_cmd": "MGC_WORKSPACE={profile_name} mgc object-storage objects acl set --dst={bucket_name}/{obj_key} --{acl_name}",
        "grant_cmd": "MGC_WORKSPACE={profile_name} mgc object-storage objects acl set --dst={bucket_name}/{obj_key} --grant-{mgc_grant_flag} id={grantee_id}"
